...
from ..losses.losses import improved_loss_progress, calculate_metrics, d_loss, g_adv_loss
...
def train_model(model, normalizer, train_loader, val_loader, shp_path, epochs=50, patience=5):
    ...
    for epoch in range(epochs):
        ...
        for batch in (train_loader):
            ...
            train_batch_count += 1
#             visualize_batch(epoch=epoch, model=model, normalizer=normalizer, dataloader=train_loader,batch_idx=0, sample_idx=0, device="cuda", save=False, train=True, shp_path=shp_path)
            
#             visualize_batch(epoch=epoch, model=model, normalizer=normalizer, dataloader=val_loader,batch_idx=0, sample_idx=0, device="cuda", save=False, train=False, shp_path=shp_path)
...
def train_gan_model(model, normalizer, train_loader, val_loader, shp_path, epochs=200, patience=5):
