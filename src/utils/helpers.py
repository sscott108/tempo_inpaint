...
# --- Final visualizer (crop-aware) with:
#     • SAME SCALE across panels
#     • Shapefile clipped to crop tile
#     • Pandora diamonds FILLED by Pandora value
#     • Proper legend handles on ax[0] (station color + fill color = value; RMSE per station if inference)
#
# Assumes each sample has:
#   masked_img[1,Hc,Wc], known_mask[1,Hc,Wc], target[1,Hc,Wc], path:str,
#   crop_origin=(row0,col0) in original, crop_size=(Hc,Wc)
# Optional: p_val_mask[1,Hc,Wc], p_mask[1,Hc,Wc]
# pandora_df columns: ['datetime','lon','lat','station', 'value'(optional)]



def _sq(t):
    if torch.is_tensor(t): t = t.detach().cpu().numpy()
    return t[0] if (isinstance(t, np.ndarray) and t.ndim == 3) else t

def _finite(a): 
    return a[np.isfinite(a)]

def _load_shapefile_segments(shp_path, dst_crs_wkt):
    try: r = shapefile.Reader(shp_path)
    except Exception: return []
    # read .prj if present; else assume already in dst
    src_wkt = None
    try:
        with open(shp_path.replace(".shp", ".prj"), "r") as f:
            src_wkt = f.read()
    except Exception:
        pass
    if src_wkt:
        tf = Transformer.from_crs(src_wkt, dst_crs_wkt, always_xy=True)
        fwd = lambda x, y: tf.transform(x, y)
    else:
        fwd = lambda x, y: (x, y)

    segs = []
    for sh in r.shapes():
        if sh.shapeType not in (3,13,23,5,15,25):  # polylines/polygons
            continue
        pts = [fwd(x, y) for x, y in sh.points]
        parts = list(sh.parts) + [len(pts)]
        for a,b in zip(parts[:-1], parts[1:]):
            if b - a >= 2:
                segs.append(np.asarray(pts[a:b], dtype=float))
    return segs

def _clip_segments_bbox(segments, xmin, xmax, ymin, ymax):
    out = []
    for seg in segments:
        x, y = seg[:,0], seg[:,1]
        if (x.max() < xmin) or (x.min() > xmax) or (y.max() < ymin) or (y.min() > ymax):
            continue
        keep = (x>=xmin)&(x<=xmax)&(y>=ymin)&(y<=ymax)
        out.append(seg[keep] if keep.any() else seg)
    return out


def _extract_sample_from_batch(batch, i):
    def _pick(v):
        ...
        if isinstance(v, (list, tuple)):
            return v[i]
        return v
    return {k: _pick(v) for k, v in batch.items()}
...
#     ax[3].axis("off")
    ...
    legend_handles = add_pandora_stations(ax[0], add_legend=True)
    if legend_handles:
        ax[0].legend(handles=legend_handles, bbox_to_anchor=(-0.70, 1.0), loc="upper left", frameon=True, fontsize=13, markerscale=1.2)
...
#     Single colorbar
    ...
    if save:
        plt.savefig(f'{epoch}.png', dpi=300, bbox_inches="tight")
